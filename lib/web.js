...
function saveCache() {
  ...
  saveTimer = setTimeout(() => {
    try { saveJSONAtomic(CACHE_PATH, cache); } catch (e) { console.error('webcache save failed:', e.message); }
  }, 300);
}

// systemic-failure signal: DDG markup changes / rate limits look identical to
// per-item errors, so track consecutive failures for the UI banner
const health = { consecFails: 0, lastError: null, lastOkAt: null };
async function tracked(fn) {
  try {
    const r = await fn();
    health.consecFails = 0;
...
function webHealth() { return { ...health }; }
...
// Google-like relevance for the image strip: results whose title contains the
// exact catalog number first, then known electrical suppliers/manufacturers,
// then everything else in engine order. Marketing banners sink naturally.
const PREFERRED_SOURCES = ['bridgeport', 'platt', 'gordonelectric', 'cesco', 'grainger', 'zoro',
  'homedepot', 'lowes', 'cooperelectric', 'worldelectricsupply', 'stokeselectric', 'radwell',
  'standardelectric', 'parrish-hare', 'crescentelectric', 'rexel', 'graybar', 'ebay', 'amazon'];
function rankImages(images, catNumber) {
  const cat = String(catNumber || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
  const score = (im) => {
    let s = 0;
    const title = String(im.title || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
    if (cat.length >= 3 && title.includes(cat)) s += 2;
    const src = `${im.source || ''} ${im.url || ''}`.toLowerCase();
    if (PREFERRED_SOURCES.some((d) => src.includes(d))) s += 1;
    return s;
  };
  return images
    .map((im, i) => ({ im, i, s: score(im) }))
    .sort((a, b) => b.s - a.s || a.i - b.i)
    .map((x) => x.im);
}

async function imageLookup(key, query, force = false, catNumber = '') {
  const k = 'img:' + key;
  // a changed query means the app now asks a better question — refetch
  if (!force && cache[k] && cache[k].images && cache[k].images.length && cache[k].query === query) return cache[k];
  const entry = { query, fetchedAt: new Date().toISOString(), images: rankImages(await tracked(() => ddgImages(query)), catNumber) };
  cache[k] = entry;
  saveCache();
  return entry;
}
...
async function webLookup(key, query, force = false) {
  ...
  const entry = { query, fetchedAt: new Date().toISOString(), results: await tracked(() => ddg(query)) };
  cache[key] = entry;
  saveCache();
  return entry;
}
...
module.exports = { webLookup, imageLookup, verifyImages, cached, cachedText, webHealth, rankImages };
