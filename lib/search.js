...
const { decodeCat } = require('./cattags');
...
function createEngine({ catalog, jargon, synonyms, mfrMap, overrides, webText }) {
  ...
  function indexItem(raw) {
    ...
    // auto-keywords, four sources so EVERY item gets some:
    // 1. the manufacturer's numbering system decoded off the catalog # (see
    // lib/cattags.js), 2. cheat-sheet rules (trade names), 3. expanded
    // description shorthand, 4. frequent words from its cached web results
    // (incl. CED portal wording)
    const autoKeywords = [];
    const autoTokens = new Set();
    const decoded = decodeCat(raw.mfr, raw.cat);
    if (decoded) {
      for (const k of decoded.keywords) {
        autoKeywords.push(k);
        for (const t of tokenize(k)) autoTokens.add(t);
      }
    }
