...
function createEngine({ catalog, jargon, synonyms, mfrMap, overrides, webText }) {
  ...
  function indexItem(raw) {
    ...
    // 4. guarantee: every item gets keywords, even with no rule hit and no web
    // cache yet — plain-English words from the abbrev-expanded description
    if (autoKeywords.length < 3) {
      const plain = [];
      for (const t of tokenize(desc)) {
        const w = (abbrev[t] || t).toLowerCase();
        if (w.length >= 4 && !/\d/.test(w) && !STOP.has(w.toUpperCase()) && !plain.includes(w)) plain.push(w);
      }
      plain.sort((a, b) => b.length - a.length);
      for (const w of plain.slice(0, 4)) {
        if (autoKeywords.includes(w)) continue;
        autoKeywords.push(w);
        for (const x of tokenize(w)) autoTokens.add(x);
      }
    }
  ...
  function toResult(it, extra) {
    return {
      ...
      image: it.image, cedImage: cedImageFor(it.upc), totalQty: it.totalQty, score: 0, why: [], ruleName: null, ...extra,
    };
  }
